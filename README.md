...
Genomes are processed in **batches**. One batch is one cluster job that runs `batch_size` genomes,
...
A working SOCfinder checkout. Follow the upstream README to:
...
Precomputed KOFAMscan tables (see the next section).
...
### Reusing scale-1.0 tables (recompute at 0.75)

KOFAMscan at the tool's default `--threshold-scale 1.0` instead of
SOCfinder's 0.75, you don't have to re-run it. `--threshold-scale` is applied after the
hmmsearch, so a scale-1.0 `detail-tsv` already contains the hits a 0.75 run would call —
they're the rows scoring between 0.75×threshold and the threshold, just left unmarked. Set:

```yaml
kofam_recompute_scale: 0.75
```

and the parser marks a hit significant when `score >= 0.75 * thrshld`, unioned with whatever
the table already starred (so no originally-significant hit is lost). SOCfinder only ever
reads that significance mark, so this reproduces 0.75 behaviour without touching the score
or threshold columns and without re-running the slowest step.

Two conditions. Your tables must be `detail-tsv` (with the score and threshold columns) — the
`mapper` format keeps only significant pairs and can't be re-thresholded. And the input must
be at scale 1.0; don't apply this to a table already made at 0.75. Validate once before
trusting it on 300k: re-run KOFAMscan at 0.75 on a few dozen genomes and confirm the
recomputed significant set matches (the rare miss is a near-threshold hit that fell below
KOFAMscan's own hmmsearch reporting cutoff and so isn't in the table at all).
